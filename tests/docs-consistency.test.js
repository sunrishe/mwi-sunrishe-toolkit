// 文档一致性回归：docs/usage.md 更新日志必须与 package.json 版本同步，
...
const usageSource = fs.readFileSync(path.join(root, 'docs', 'usage.md'), 'utf8');

function changelogVersionHeadings() {
  return [
    ...usageSource.matchAll(/^## (v\d+\.\d+\.\d+)$/gm)
  ].map((match) => match[1]);
}

test('usage.md 更新日志最新条目与 package.json 版本一致', () => {
